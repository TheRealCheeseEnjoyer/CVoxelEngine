...
static int  thread_init(struct thread** thread_p, int id);
...
void thpool_init(int num_threads){
	threads_on_hold   = 0;
	threads_keepalive = 1;
	...
	if (jobqueue_init(&queue) == -1){
		err("thpool_init(): Could not allocate memory for job queue\n");
		return;
	}

	/* Make threads in pool */
	threads = (struct thread**)malloc(num_threads * sizeof(struct thread *));
	if (threads == NULL){
...
void thpool_wait(){
	...
	while (queue.len || num_threads_working) {
...
void thpool_destroy(){
	volatile int threads_total = num_threads_alive;
	...
	double tpassed = 0.0;
	time (&start);
	while (tpassed < TIMEOUT && num_threads_alive){
...
int thpool_num_threads_working(){
